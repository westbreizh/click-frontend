...
export default function ProductBallPage() {
  ...
  const loadProductSelected  = async function (data) {
    try{
      const response = await fetch(`https://click-backend.herokuapp.com/api/shop/productSelected`, {
        mode: "cors",
        method: "POST",
        body: JSON.stringify({ productCategorie: "ball", productId: productId }),
        ...
        {productSelected.length > 0 ? 
          ...
          (
            <>
              ...
              <h1 className="title-product"> Balles - {product.mark} - {product.model} </h1>
